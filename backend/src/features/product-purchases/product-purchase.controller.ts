    ...
    const data = await prisma.$transaction(async (tx) => {
      ...
      const purchase = await tx.productPurchase.create({
        data: {
          ...
          salonId,
          ...(branchId ? { branchId } : {}),
          ...(typeof req.body.supplierName === "string" && req.body.supplierName.trim() ? { supplierName: req.body.supplierName.trim() } : {}),
          ...(typeof req.body.supplierPhone === "string" && req.body.supplierPhone.trim() ? { supplierPhone: req.body.supplierPhone.trim() } : {}),
